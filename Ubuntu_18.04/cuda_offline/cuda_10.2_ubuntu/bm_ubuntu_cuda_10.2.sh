...
set -e
#####################
## Install CUDA 10.2 on Ubuntu 18.04 64-bit LTS
##
## Usage:
##  1. put the packages downloaded from https://developer.nvidia.com/cuda-downloads
##     in the folder of this script
##  2. use './bm_ubuntu_cuda_10.2.sh' to install
##
## on ubuntu 18.04 ,need --override to skip 'cuda Error: unsupported compiler: 7.3.0.'
## Last tested on Ubuntu 18.04 64-bit LTS with "cuda_10.2.89_440.33.01_linux.run"
#####################

# files to be used
export BM_CUDA_RUN=cuda_10.2.89_440.33.01_linux.run
export BM_CUDA_PROFILE=/etc/profile.d/bm_cuda_10.2.sh
...
export BM_OPTION_CUDA_SAMPLEPATH=$HOME/NVIDIA_CUDA-10.2_Samples
...
echoWarning "===BM: Script to install CUDA 10.2 (without driver) and its patchs==="
# Usage
...
echoNotify ">>>BM: install CUDA 10.2 using $BM_CUDA_RUN ..."
...
sudo bash -c "echo '#added by $USER, to install CUDA 10.2' > $BM_CUDA_PROFILE"
...
echoNotify ">>>>>>BM: CUDA 10.2 toolkit's installation folder is $BM_OPTION_CUDA_TOOLKITPATH"
